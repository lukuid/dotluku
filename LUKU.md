...
| Type | Description |
| :--- | :--- |
| `scan` | A physical interaction record (RFID/NFC scan). Covers multiple hardware **profiles** (e.g. animal microchip reads, access-credential reads) under one native record type; see "The `scan` Record & Scan Profiles" below. |
...
*   **`scan`**: Represents a physical interaction with a tag, chip, or credential (RFID/NFC or equivalent), such as reading a microchip with the Animal Reader or reading an access credential at a door controller. Hardware-specific field sets are declared as named `profile` variants of this single type; see "The `scan` Record & Scan Profiles" below.
*   **`environment`**: Captures sensor telemetry and tamper events, such as those generated by the LukuID Environment.
...
#### The `scan` Record & Scan Profiles

`scan` is a native, chain-advancing record type for **any** device-attested physical interaction with a tag, chip, or credential (RFID/NFC or equivalent). Rather than defining one fixed payload, `scan` declares a required `profile` field that selects a versioned, named field set for that hardware use case (e.g. `animal` for the Animal Reader, `access` for a door/access controller). All profiles share the same continuity, counter, genesis, and chain-linkage rules that apply to native `scan` records — adding a profile does **not** require touching any of the continuity/counter/genesis language elsewhere in this spec, and does **not** introduce a new `type`.

**Fixed (common) canonical fields**, present for every profile, in this order:
`device_id:public_key:scan:id:ctr:timestamp_utc:uptime_us:profile:nonce:firmware`

**Profile-specific canonical fields**: each profile declares its own named field set. Those field values MUST be appended, in **strict lexicographic (byte-wise ASCII) order of their field names**, immediately after `firmware` and before the trailing common fields below. This is a mechanical rule, not hand-authored prose — a new profile is defined purely by declaring its field-name list (and each field's type/serialization per the standard rules); no bespoke "MUST follow this order" sentence is required. A field absent for a given profile instance MUST be serialized per the standard Canonical Serialization Rules (empty string for missing string/optional fields) — see the **No Silent Numeric Defaults** rule below; it MUST NOT be defaulted to `0`.

**Trailing (common) canonical fields**, present for every profile, in this order:
`...:metrics:previous_signature`

So the full canonical template is:
`device_id:public_key:scan:id:ctr:timestamp_utc:uptime_us:profile:nonce:firmware:{profile fields, alphabetical by name}:metrics:previous_signature`

A verifier that encounters a `profile` value it does not recognize MUST treat the record as having an unverifiable/unknown schema (the same posture as encountering an unrecognized `type`) rather than guessing a field layout or omitting fields from the recomputation.
...
Profile-specific fields, alphabetical: `asset_id, credential_id, credential_type, protocol, result`.
...
```json
{
  ...
  "batch": [
    {
      "type": "scan",
      "id": "LUKUID-1770823456-9001-doorlock",
      "version": "1.0.0",
      "alg": "ED25519",
      "signature": "base64_payload_signature",
      "previous_signature": "base64_previous_record_signature",
      "canonical_string": "LUK-DOOR-042:base64_device_public_key:scan:LUKUID-1770823456-9001-doorlock:9001:1770823456:88000000:access:door_challenge_token_abc:DL-2.1.0:MAIN-ENTRANCE-02:CRED-55321:mifare_desfire:ISO14443A:granted::base64_previous_record_signature",
      "payload": {
        "ctr": 9001,
        "timestamp_utc": 1770823456,
        "uptime_us": 88000000,
        "nonce": "door_challenge_token_abc",
        "firmware": "DL-2.1.0",
        "profile": "access",
        "asset_id": "MAIN-ENTRANCE-02",
        "credential_id": "CRED-55321",
        "credential_type": "mifare_desfire",
        "protocol": "ISO14443A",
        "result": "granted",
        "metrics": []
      },
  ...
  "batch_hash": "sha256_hex_batch_hash_for_block_0",
  "block_canonical_string": "0:1770823456::LUK-DOOR-042:base64_device_public_key:sha256_hex_fingerprint:sha256_hex_fingerprint:sha256_hex_batch_hash_for_block_0",
  "block_hash": "sha256_hex_block_hash_for_block_0"
}
```

*   **asset_id**: The controlled point or asset the read was evaluated against (e.g. a door, gate, cabinet, or vehicle lock) — generic on purpose, since the field's meaning is scoped to the deployment, not the protocol.
*   **credential_id / credential_type**: The scanned credential's identifier and medium (e.g. `mifare_desfire`, `ble_mobile_key`).
*   **result**: `granted` or `denied`. This is the outcome of the access decision, not proof that a person passed through.
*   **metrics**: Empty for this profile unless a future minor revision defines diagnostic values for it; still present and serialized as an empty array per the fixed canonical template.

##### Adding a New Scan Profile

A new hardware use case is introduced by declaring, for that `profile` value:

1.  The profile-specific field-name list and each field's type/serialization (per the standard Canonical Serialization Rules).
2.  Nothing else — ordering is derived automatically (alphabetical by field name), the common prefix/suffix fields are unchanged, and no edits are required to the continuity, counter, genesis, or chain-linkage rules, since the record remains `type: "scan"`.

This keeps `scan` a single, extensible native record type instead of requiring a new `type` (with its own continuity-rule wiring) for every new interaction-scanning product line.

#### The `environment` Record (e.g., LukuID Environment)
